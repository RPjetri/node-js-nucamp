            ...
            .then(favorite => {
                if (favorite) {
                    ...
                    Favorite.create({ user: req.user._id })
                        .then(favorite => {
                            req.body.forEach(fav => {
                                if (!favorite.campites.includes(fav._id)) {
...
favoriteRouter.route('/:campsiteId')
            ...
            .then(favorite => {
                if (favorite) {
                    if (!favorite.campsites.includes(req.params.campsiteId)) {
                        favorites.campites.push(req.params.campsiteId);
                        favorite.save()
                            .then(favorite => {
                                res.statusCode = 200;
                                res.setHeader('Content-Type', 'application/json');
    ...
    .put(cors.corsWithOptions, authenticate.verifyUser, (req, res) => {
        res.statusCode = 403;
        res.end(`PUT operaton not supported on /favorites/${req.params.campsiteId}`);
    })
